...
class field_value_validators {
    ...
    public static function validate_theme($value) {
        ...
        if (!$CFG->allowuserthemes) {
            $status = 'warning';
            $message = get_string('userthemesnotallowed', 'tool_uploadusercli');
        } else {
            ...
            if (empty($value)) {
                $status = 'warning';
                $message = get_string('notheme', 'tool_uploadusercli');
            } else if (!isset(self::$themescache[$value])) {
                $status = 'warning';
                $message = get_string('invalidtheme', 'tool_uploadusercli', s($value));
            }
        }
