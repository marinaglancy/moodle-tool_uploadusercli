...
require_once($CFG->dirroot.'/'.$CFG->admin.'/tool/uploadusercli/user_form.php');
...
class cli_helper {
    ...
    protected function options_definitions(): array {
        $options = [
            'help' => [
                'hasvalue' => false,
                'description' => get_string('clihelp', 'tool_uploadusercli'),
                'default' => 0,
                'alias' => 'h',
            ],
            'file' => [
                'hasvalue' => 'PATH',
                'description' => get_string('clifile', 'tool_uploadusercli'),
                'default' => null,
                'validation' => function($file) {
                    if (!$file) {
                        $this->cli_error(get_string('climissingargument', 'tool_uploadusercli', 'file'));
                    }
    ...
    public function print_help(): void {
        $this->cli_writeln(get_string('clititle', 'tool_uploadusercli'));
        $this->cli_writeln('');
        $this->print_help_options($this->options_definitions());
        $this->cli_writeln('');
        $this->cli_writeln('Example:');
        $this->cli_writeln('$sudo -u www-data /usr/bin/php admin/tool/uploadusercli/cli/uploadusercli.php --file=PATH');
    }
    ...
    public function process(): void {
        ...
        // Read the CSV file.
        $iid = \csv_import_reader::get_new_iid('uploadusercli');
        $cir = new \csv_import_reader($iid, 'uploadusercli');
        ...
        if (!$form->is_validated()) {
            ...
            $this->cli_error(get_string('clivalidationerror', 'tool_uploadusercli') . PHP_EOL .
    ...
    protected function prepare_form_elements_for_cli(array $elements, array $defaults): array {
        ...
        foreach ($elements as $element) {
            ...
            if ($default !== null & $default !== '') {
                $postfix .= "\n  ".get_string('clidefault', 'tool_uploadusercli')." ".$default;
            }
            $options[$name] = [
                'hasvalue' => 'VALUE',
            ...
            if ($possiblevalues !== null) {
                $options[$name]['validation'] = function($v) use ($possiblevalues, $name) {
                    if (!in_array('' . $v, $possiblevalues)) {
                        $this->cli_error(get_string('clierrorargument', 'tool_uploadusercli',
