...
namespace tool_uploadusercli;

defined('MOODLE_INTERNAL') || die();

use tool_uploadusercli\local\field_value_validators;
...
class process {
    ...
    public function process_line(array $line) {
        ...
        if ($user->mnethostid == $CFG->mnet_localhost_id) {
            ...
            if (!$existinguser or $this->get_operation_type() == UU_USER_ADDINC) {
                $this->upt->track('status', get_string('errormnetadd', 'tool_uploadusercli'), 'error');
                $this->userserrors++;
                return;
            }
        ...
        if (!empty($user->deleted)) {
            ...
            if ($existinguser) {
                ...
                if (delete_user($existinguser)) {
                    $this->upt->track('status', get_string('userdeleted', 'tool_uploadusercli'));
                    $this->deletes++;
                } else {
        ...
        if (!empty($user->oldusername) ) {
            ...
            if ($olduser = $DB->get_record('user',
                ...
                $this->upt->track('status', get_string('userrenamed', 'tool_uploadusercli'));
                $this->renames++;
            } else {
        ...
        if ($existinguser) {
            ...
            if ($doupdate or $existinguser->password !== $oldpw) {
                ...
                $this->upt->track('status', get_string('useraccountupdated', 'tool_uploadusercli'));
                $this->usersupdated++;
                ...
                $this->upt->track('status', get_string('useraccountuptodate', 'tool_uploadusercli'));
                $this->usersuptodate++;
            ...
            if ($isinternalauth) {
                if (empty($user->password)) {
                    if ($this->get_create_paswords()) {
                        ...
                        $this->upt->track('password', '', 'normal', false);
                        $this->upt->track('password', get_string('uupasswordcron', 'tool_uploadusercli'), 'warning', false);
                    } else {
                        $this->upt->track('password', '', 'normal', false);
                    ...
                    if ($removing) {
                        if (user_has_role_assignment($user->id, $sysroleid, SYSCONTEXTID)) {
                            ...
                            $this->upt->track('enrolments', get_string('unassignedsysrole',
                                'tool_uploadusercli', $this->sysrolecache[$sysroleid]->name), 'info');
                        }
                    } else {
                        if (!user_has_role_assignment($user->id, $sysroleid, SYSCONTEXTID)) {
        ...
        if (($invalid = \core_user::validate($user)) !== true) {
            $this->upt->track('status', get_string('invaliduserdata', 'tool_uploadusercli', s($user->username)), 'warning');
        }
    }
    ...
    public function get_stats() {
        ...
        if ($this->get_operation_type() != UU_USER_UPDATE) {
            $lines[] = get_string('userscreated', 'tool_uploadusercli').': '.$this->usersnew;
        }
        if ($this->get_operation_type() == UU_USER_UPDATE or $this->get_operation_type() == UU_USER_ADD_UPDATE) {
            $lines[] = get_string('usersupdated', 'tool_uploadusercli').': '.$this->usersupdated;
        }
        if ($this->get_allow_deletes()) {
            $lines[] = get_string('usersdeleted', 'tool_uploadusercli').': '.$this->deletes;
            $lines[] = get_string('deleteerrors', 'tool_uploadusercli').': '.$this->deleteerrors;
        }
        if ($this->get_allow_renames()) {
            $lines[] = get_string('usersrenamed', 'tool_uploadusercli').': '.$this->renames;
            $lines[] = get_string('renameerrors', 'tool_uploadusercli').': '.$this->renameerrors;
        }
        if ($usersskipped = $this->usersskipped) {
            $lines[] = get_string('usersskipped', 'tool_uploadusercli').': '.$usersskipped;
        }
        $lines[] = get_string('usersweakpassword', 'tool_uploadusercli').': '.$this->weakpasswords;
        $lines[] = get_string('errors', 'tool_uploadusercli').': '.$this->userserrors;
