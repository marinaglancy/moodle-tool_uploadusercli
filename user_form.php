...
class tool_uploadusercli_admin_uploaduser_form1 extends moodleform {
    function definition () {
        ...
        $mform->addElement('static', 'examplecsv', get_string('examplecsv', 'tool_uploadusercli'), $link);
        $mform->addHelpButton('examplecsv', 'examplecsv', 'tool_uploadusercli');

        $mform->addElement('filepicker', 'userfile', get_string('file'));
        $mform->addRule('userfile', null, 'required');

        $choices = csv_import_reader::get_delimiter_list();
        $mform->addElement('select', 'delimiter_name', get_string('csvdelimiter', 'tool_uploadusercli'), $choices);
        ...
        $mform->addElement('select', 'encoding', get_string('encoding', 'tool_uploadusercli'), $choices);
        $mform->setDefault('encoding', 'UTF-8');

        $choices = array('10'=>10, '20'=>20, '100'=>100, '1000'=>1000, '100000'=>100000);
        $mform->addElement('select', 'previewrows', get_string('rowpreviewnum', 'tool_uploadusercli'), $choices);
        $mform->setType('previewrows', PARAM_INT);

        $this->add_action_buttons(false, get_string('uploadusers', 'tool_uploadusercli'));
    }
...
class tool_uploadusercli_admin_uploaduser_form2 extends moodleform {
    function definition () {
        ...
        $choices = array(UU_USER_ADDNEW     => get_string('uuoptype_addnew', 'tool_uploadusercli'),
                         UU_USER_ADDINC     => get_string('uuoptype_addinc', 'tool_uploadusercli'),
                         UU_USER_ADD_UPDATE => get_string('uuoptype_addupdate', 'tool_uploadusercli'),
                         UU_USER_UPDATE     => get_string('uuoptype_update', 'tool_uploadusercli'));
        $mform->addElement('select', 'uutype', get_string('uuoptype', 'tool_uploadusercli'), $choices);

        $choices = array(0 => get_string('infilefield', 'auth'), 1 => get_string('createpasswordifneeded', 'auth'));
        $mform->addElement('select', 'uupasswordnew', get_string('uupasswordnew', 'tool_uploadusercli'), $choices);
        $mform->setDefault('uupasswordnew', 1);
        $mform->hideIf('uupasswordnew', 'uutype', 'eq', UU_USER_UPDATE);

        $choices = array(UU_UPDATE_NOCHANGES    => get_string('nochanges', 'tool_uploadusercli'),
                         UU_UPDATE_FILEOVERRIDE => get_string('uuupdatefromfile', 'tool_uploadusercli'),
                         UU_UPDATE_ALLOVERRIDE  => get_string('uuupdateall', 'tool_uploadusercli'),
                         UU_UPDATE_MISSING      => get_string('uuupdatemissing', 'tool_uploadusercli'));
        $mform->addElement('select', 'uuupdatetype', get_string('uuupdatetype', 'tool_uploadusercli'), $choices);
        ...
        $choices = array(0 => get_string('nochanges', 'tool_uploadusercli'), 1 => get_string('update'));
        $mform->addElement('select', 'uupasswordold', get_string('uupasswordold', 'tool_uploadusercli'), $choices);
        ...
        $choices = array(UU_PWRESET_WEAK => get_string('usersweakpassword', 'tool_uploadusercli'),
        ...
        $mform->addElement('selectyesno', 'uuallowrenames', get_string('allowrenames', 'tool_uploadusercli'));
        ...
        $mform->addElement('selectyesno', 'uuallowdeletes', get_string('allowdeletes', 'tool_uploadusercli'));
        ...
        $mform->addElement('selectyesno', 'uuallowsuspends', get_string('allowsuspends', 'tool_uploadusercli'));
        ...
        if (!empty($CFG->allowaccountssameemail)) {
            $mform->addElement('selectyesno', 'uunoemailduplicates', get_string('uunoemailduplicates', 'tool_uploadusercli'));
        ...
        $mform->addElement('selectyesno', 'uustandardusernames', get_string('uustandardusernames', 'tool_uploadusercli'));
        $mform->setDefault('uustandardusernames', 1);

        $choices = array(UU_BULK_NONE    => get_string('no'),
                         UU_BULK_NEW     => get_string('uubulknew', 'tool_uploadusercli'),
                         UU_BULK_UPDATED => get_string('uubulkupdated', 'tool_uploadusercli'),
                         UU_BULK_ALL     => get_string('uubulkall', 'tool_uploadusercli'));
        $mform->addElement('select', 'uubulk', get_string('uubulk', 'tool_uploadusercli'), $choices);
        ...
        if ($showroles) {
            ...
            $mform->addElement('select', 'uulegacy2', get_string('uulegacy2role', 'tool_uploadusercli'), $choices);
            ...
            $mform->addElement('select', 'uulegacy3', get_string('uulegacy3role', 'tool_uploadusercli'), $choices);
        ...
        // default values
        $mform->addElement('header', 'defaultheader', get_string('defaultvalues', 'tool_uploadusercli'));

        $mform->addElement('text', 'username', get_string('uuusernametemplate', 'tool_uploadusercli'), 'size="20"');
        $mform->setType('username', PARAM_RAW); // No cleaning here. The process verifies it later.
        $mform->addRule('username', get_string('requiredtemplate', 'tool_uploadusercli'), 'required', null, 'client');
        ...
        $this->add_action_buttons(true, get_string('uploadusers', 'tool_uploadusercli'));
    ...
    function validation($data, $files) {
        ...
        if (!empty($updatetype) && ($optype == UU_USER_ADDNEW || $optype == UU_USER_ADDINC)) {
            $errors['uuupdatetype'] = get_string('invalidupdatetype', 'tool_uploadusercli');
        }
        ...
        if ($optype != UU_USER_UPDATE) {
            ...
            if (!in_array('email', $columns) and empty($data['email'])) {
                $errors['email'] = get_string('requiredtemplate', 'tool_uploadusercli');
            }
        }
        return $errors;
    }
