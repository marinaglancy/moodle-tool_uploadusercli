 ...
 *
 * Based on .../admin/uploadusercli.php and .../lib/gdlib.php
 *
 * @package    tool
...
require_capability('tool/uploadusercli:uploaduserpictures', context_system::instance());
...
$strusersupdated = get_string('usersupdated', 'tool_uploadusercli');
$struploadpictures = get_string('uploadpictures','tool_uploadusercli');
...
echo $OUTPUT->heading_with_help($struploadpictures, 'uploadpictures', 'tool_uploadusercli');

$mform = new admin_uploadpicture_form(null, $userfields);
if ($formdata = $mform->get_data()) {
    if (!array_key_exists($userfield, $userfields)) {
        echo $OUTPUT->notification(get_string('uploadpicture_baduserfield', 'tool_uploadusercli'));
    } else {
        ...
        if (!$mform->save_file('userpicturesfile', $dstfile, true)) {
            echo $OUTPUT->notification(get_string('uploadpicture_cannotmovezip', 'tool_uploadusercli'));
            @remove_dir($zipdir);
        } else {
            $fp = get_file_packer('application/zip');
            $unzipresult = $fp->extract_to_pathname($dstfile, $zipdir);
            if (!$unzipresult) {
                echo $OUTPUT->notification(get_string('uploadpicture_cannotunzip', 'tool_uploadusercli'));
                @remove_dir($zipdir);
            } else {
                ...
                remove_dir($zipdir);
                echo $OUTPUT->notification(get_string('usersupdated', 'tool_uploadusercli') . ": " . $results['updated'], 'notifysuccess');
                echo $OUTPUT->notification(get_string('errors', 'tool_uploadusercli') . ": " . $results['errors'], ($results['errors'] ? 'notifyproblem' : 'notifysuccess'));
                echo '<hr />';
            }
        }
    }
...
function process_directory ($dir, $userfield, $overwrite, &$results) {
    ...
    if(!($handle = opendir($dir))) {
        echo $OUTPUT->notification(get_string('uploadpicture_cannotprocessdir', 'tool_uploadusercli'));
        return;
    }
...
function process_file ($file, $userfield, $overwrite) {
    ...
    if (!($user = $DB->get_record('user', array ($userfield => $uservalue, 'deleted' => 0)))) {
        ...
        echo $OUTPUT->notification(get_string('uploadpicture_usernotfound', 'tool_uploadusercli', $a));
        return PIX_FILE_ERROR;
    }

    $haspicture = $DB->get_field('user', 'picture', array('id'=>$user->id));
    if ($haspicture && !$overwrite) {
        echo $OUTPUT->notification(get_string('uploadpicture_userskipped', 'tool_uploadusercli', $user->username));
        return PIX_FILE_SKIPPED;
    }

    if ($newrev = my_save_profile_image($user->id, $file)) {
        $DB->set_field('user', 'picture', $newrev, array('id'=>$user->id));
        echo $OUTPUT->notification(get_string('uploadpicture_userupdated', 'tool_uploadusercli', $user->username), 'notifysuccess');
        return PIX_FILE_UPDATED;
    } else {
        echo $OUTPUT->notification(get_string('uploadpicture_cannotsave', 'tool_uploadusercli', $user->username));
        return PIX_FILE_ERROR;
    }
}
