...
class admin_uploadpicture_form extends moodleform {
    function definition (){
        ...
        $mform->addElement('select', 'userfield', get_string('uploadpicture_userfield', 'tool_uploadusercli'), $choices);
        $mform->setType('userfield', PARAM_INT);

        $choices = array( 0 => get_string('no'), 1 => get_string('yes') );
        $mform->addElement('select', 'overwritepicture', get_string('uploadpicture_overwrite', 'tool_uploadusercli'), $choices);
        $mform->setType('overwritepicture', PARAM_INT);

        $this->add_action_buttons(false, get_string('uploadpictures', 'tool_uploadusercli'));
    }
}
